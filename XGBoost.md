...
**一个好的分裂，本质上是让左右子节点更“各自适合一个预测值”。XGBoost做的事情，就是把这种“适合程度”量化。 但它不是像普通分类树那样只看基尼系数或信息增益，它直接从目标函数优化的角度来算。这也是XGBoost特别强的地方。**
...
**缺失值填充这种需要从数据中计算统计量的操作，应该放在划分训练集和测试集之后进行。否则如果直接用完整数据计算中位数，就相当于提前使用了测试集的信息，容易产生数据泄漏。**——使用测试集
