...
class RegressionAnalysis:
    ...
    def train_mlr(self, X_train, y_train):
        ...
        return self._fit_grid('MLR', LinearRegression(), param_grid, X_train, y_train, scale=True)

    # ElasticNet
    def train_elasticnet(self, X_train, y_train):
        en = ElasticNet(random_state=self.random_state, max_iter=5000)
        param_grid = {
            'reg__alpha': [1e-3, 1e-2, 1e-1, 1.0, 10.0],
            'reg__l1_ratio': [0.1, 0.3, 0.5, 0.7, 0.9],
        }
        return self._fit_grid("ElasticNet", en, param_grid, X_train, y_train, scale=True)
    ...
    def run_analysis(self):
        ...
        trainers = [self.train_mlr, self.train_elasticnet]
