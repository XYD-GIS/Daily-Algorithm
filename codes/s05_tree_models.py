...
DATA_PATH = None
GROUPS_PATH = None
MODEL_GROUP = 's05_tree_models'
N_JOBS = min(4, os.cpu_count() or 1)
OUTPUT_DIR = None


# 模型兼容
class CompatibleCatBoostRegressor(cb.CatBoostRegressor, RegressorMixin, BaseEstimator):
...
class RegressionAnalysis:
    ...
    def _evaluate_model(self, est, model_name, X_train, X_test, y_train, y_test):
        ...
        fig.tight_layout()
        fig.savefig(self.output_dir / f'{model_name}_actual_predicted.png', dpi=600)
    ...
    def train_random_forest(self, X_train, y_train):
        ...
        return self._fit_grid(
            ...
            param_grid, X_train, y_train, scale=False
        )
    ...
    def train_extra_trees(self, X_train, y_train):
        ...
        param_grid = {
            'reg__n_estimators': [200, 400, 600, 800],
            'reg__max_depth': [None, 8, 12, 20],
            'reg__min_samples_leaf': [1, 2, 4],
            'reg__max_features': ['sqrt', 'log2', None],
        }
        return self._fit_grid("ExtraTrees", et, param_grid, X_train, y_train, scale=False)
    ...
    def train_gbdt(self, X_train, y_train):
        ...
        return self._fit_grid("GBDT", GradientBoostingRegressor(random_state=self.random_state),
                              param_grid, X_train, y_train, scale=False)
    ...
    def train_xgboost(self, X_train, y_train):
        ...
        return self._fit_grid('XGBoost', xgb_reg, param_grid, X_train, y_train, scale=False)

    # LightGBM
    def train_lightgbm(self, X_train, y_train):
        lgb_reg = lgb.LGBMRegressor(random_state=self.random_state, n_jobs=1, subsample_freq=1, verbosity=-1)
        param_grid = {
            'reg__n_estimators': [300, 600, 900],
            'reg__num_leaves': [31, 63, 127],
            'reg__max_depth': [-1, 6, 10],
            'reg__learning_rate': [0.01, 0.05, 0.1],
            'reg__subsample': [0.7, 0.85, 1.0],
            'reg__feature_fraction': [0.7, 0.85, 1.0],
        }
        return self._fit_grid("LightGBM", lgb_reg, param_grid, X_train, y_train, scale=False)

    # CatBoost
    def train_catboost(self, X_train, y_train):
        cat_reg = CompatibleCatBoostRegressor(random_state=self.random_state, verbose=False, thread_count=1, allow_writing_files=False)
        param_grid = {
            'reg__iterations': [400, 800, 1200],
            'reg__depth': [4, 6, 8],
            'reg__learning_rate': [0.01, 0.03, 0.06, 0.1],
            'reg__l2_leaf_reg': [1, 3, 5, 7],
        }
        return self._fit_grid("CatBoost", cat_reg, param_grid, X_train, y_train, scale=False)
    ...
    def train_adaboost(self, X_train, y_train):
        ...
        return self._fit_grid("AdaBoost", ada, param_grid, X_train, y_train, scale=False)
    ...
    def run_analysis(self):
        ...
        trainers = [self.train_random_forest, self.train_extra_trees, self.train_gbdt, self.train_xgboost, self.train_lightgbm, self.train_catboost, self.train_adaboost]
    ...
    def save_results_to_excel(self):
        ...
        df = df.sort_values('CV_RMSE', ascending=True)
        ...
        filename = self.output_dir / f'Model_Results_{timestamp}.xlsx'

        with pd.ExcelWriter(str(filename)) as writer:
            df.to_excel(writer, sheet_name='模型性能汇总', index=False)

            analysis = pd.DataFrame({
                '指标': ['最佳模型', '最差模型', '平均测试R2', 'R2范围', '平均过拟合程度'],
                '值': [
                    df.iloc[0]['Model'],
                    df.iloc[-1]['Model'],
                    f"{df['Test_R2'].mean():.3f}",
                    f"{df['Test_R2'].min():.3f} - {df['Test_R2'].max():.3f}",
